...
def load_model(
    ...
    Uses `from_pretrained_no_processing`: in reduced precision TransformerLens
    advises against the default weight-processing path (LayerNorm folding,
    weight centering), which spikes memory on load and is numerically shaky in
    bf16. Skipping it also keeps the cached residual stream identical to the raw
    HF model — the honest target for a probing study.
